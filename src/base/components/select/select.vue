<template>
    <div :class="classes" v-clickoutside="handleClose">
        <div :class="selectionCls" ref="reference" @click="toggleMenu($event)">
            <slot name="input">
                <div class="ivu-tag" v-for="(item, index) in selectedMultiple" :title.prop="item.label">
                ...
                <input type="text" :readonly="readonly" v-if="filterable" v-model="query" :disabled="disabled" :class="[prefixCls + '-input']" :placeholder="showPlaceholder ? localePlaceholder : ''" :style="inputStyle" @blur="handleBlur" @keydown="resetInputState" @keydown.delete="handleInputDelete" ref="input">
                <Icon type="ios-close" :class="[prefixCls + '-arrow']" v-show="showCloseIcon" @click.native.stop="clearSingleSelect"></Icon>
                <Icon type="arrow-down-b" v-show="!readonly" :class="[prefixCls + '-arrow']" v-if="!remote"></Icon>
            </slot>
        </div>
        <transition :name="transitionName">
            <Drop :class="dropdownCls" v-show="dropVisible" :placement="pplacement" ref="dropdown" :data-transfer="transfer" v-transfer-dom>
...
import { oneOf, findComponentDownward, getElementViewTop, getElementViewLeft } from '../../utils/assist';
...
export default {
    ...
    methods: {
        toggleMenu($event) {
            // if (document.body.offsetHeight - getElementViewTop($event.target) < (this.options.length * 21)){
            //     this.placement = 'top';
            ...
            if (document.body.clientHeight < ($event.clientY + 210)) {
                this.pplacement = 'top';
            } else {
                this.pplacement = 'bottom';
            }
            // console.log(this.pplacement);
